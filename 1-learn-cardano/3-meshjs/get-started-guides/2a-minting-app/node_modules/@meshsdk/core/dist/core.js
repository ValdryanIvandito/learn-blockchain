...
const Ze = ["testnet", "preview", "preprod", "mainnet"], ts = (s) => Ze.includes(s), Ee = {
  ...
  return [
    ...i.less_than(r) ? As(r.clamped_sub(i).to_str(), e.filter((c) => !a.some((u) => u.input.txHash === c.input.txHash && u.input.outputIndex === c.input.outputIndex))) : [],
    ...a
  ];
...
D.prototype.digest = function(s) {
  if (K(this.finalized === !1, "Hash instance finalized"), this.finalized = !0, Ct.push(this.pointer), f.exports.blake2b_final(this.pointer), !s || s === "binary")
...
const Ns = Ks(Q.exports), pt = nt, Ut = "asset";
class gt {
  constructor(e) {
    this.hashBuf = e;
  }
...
const Fs = Ps, Ts = (s, e) => {
  ...
  return t.free(), r;
}, mr = (s) => ht(s).to_hex(), pr = (s, e = Date.now()) => {
  if (xt[s]) {
    const [t, r, a] = xt[s];
...
let L = class {
  ...
  async addBurnInputsIfNeeded() {
    if (this._initiator && this._totalBurns.size > 0 && this.notVisited("setTxInputs")) {
      ...
      is(this._totalBurns, e.map((r) => Qt(r))).map((r) => N(r)).forEach((r) => {
  ...
  async addTxInputsAsNeeded() {
    if (this.notVisited("setTxInputs")) {
      ...
      ns(this._txOutputs, e.map((r) => Qt(r))).map((r) => N(r)).forEach((r) => {
        this._txInputsBuilder.add_input(r.output().address(), r.input(), r.output().amount());
      });
    }
      ...
      const u = (p, E = [], w = 0) => {
        ...
        switch (v.kind()) {
          case n.CertificateKind.StakeDeregistration: {
            const y = (It = v.as_stake_deregistration()) == null ? void 0 : It.stake_credential(), O = (y == null ? void 0 : y.kind()) === n.StakeCredKind.Key ? y.to_keyhash() : void 0;
            O && k.push(O.to_hex());
            break;
          }
        ...
        if (p === void 0 || w >= p.len())
          return E;
        const v = p.get(w).index(), k = p.get(w).transaction_id(), It = t.find((ct) => ct.input.outputIndex === v && ct.input.txHash === k.to_hex()) !== void 0 ? r : "OUR_PRINCESS_IS_IN_ANOTHER_CASTLE";
        return A(p, [...E, It], w + 1);
...
class Ft {
  ...
  async getAssets() {
    return (await this.getBalance()).filter((t) => t.unit !== "lovelace").map((t) => {
      const r = t.unit.slice(0, F), a = t.unit.slice(F), i = Ws(r, a);
      return {
        unit: t.unit,
        policyId: r,
        assetName: Yt(a),
